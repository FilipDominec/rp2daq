...
A: Perhaps, but - the firmware and computer communicate over a binary interface that would have to be ported to this language. One of the strong points of RP2DAQ is that the interface on the computer side is autogenerated; such feature would take some effort to be ported. Hard-coding the messages is an easier option, but it would be bound to a single firmware version. Python is fine.
