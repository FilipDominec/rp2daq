...
void pwm_configure_pair() {
	struct __attribute__((packed)) {
		uint8_t pin;				// default=0		min=0		max=31
		uint16_t wrap_value;		// default=999		min=1		max=65535
		uint16_t clkdiv;			// default=1		min=1		max=65535
		uint16_t clkdiv_int_frac;	// default=0		min=0		max=9
		// clkdiv of 833 yields 300 Hz (at wrap=999) - suitable for servos
    ...
    pwm_set_clkdiv_int_frac(slice_num, args->clkdiv, args->clkdiv_decimal);
...
void pwm_set_value() {
	struct __attribute__((packed)) {
		uint8_t pin;				// default=0		min=0		max=31
		uint8_t value;				// default=0		min=0		max=1
	} * args = (void*)(command_buffer+1);
	...
	tx_header_and_data(&pwm_set_value_report, sizeof(pwm_set_report), 0, 0, 0);
}
